...
public class ExcelUtil {  
     ...
     * */
    public static Workbook createWorkBook(List<ExportPermissionInfoVo> list,String []keys,String columnNames[]) {
        // 创建excel工作簿
...
//      f3.setColor(IndexedColors.RED.getIndex());
        ...
        for (short i = 1; i <= list.size(); i++) {
            ...
            // 创建一行，在页sheet上
            Row nextRow = sheet.createRow((short) i);
            // 在row行上创建一个方格
            cell=nextRow.createCell(0);
			cell.setCellValue(list.get(i-1).getId());
			cell.setCellStyle(cs2);
			cell=nextRow.createCell(1);
			cell.setCellValue(list.get(i-1).getJobnum());
			cell.setCellStyle(cs2);
			cell=nextRow.createCell(2);
			cell.setCellValue(list.get(i-1).getMobile());
			cell.setCellStyle(cs2);
			cell=nextRow.createCell(3);
			cell.setCellValue(list.get(i-1).getResult());
			cell.setCellStyle(cs2);
			cell=nextRow.createCell(4);
			cell.setCellValue(list.get(i-1).getReason());
			cell.setCellStyle(cs2);
        }
        return wb;
    }
