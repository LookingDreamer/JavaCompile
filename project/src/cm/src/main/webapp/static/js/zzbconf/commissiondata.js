require(["jquery", "bootstrap-table", "bootstrap","bootstrapTableZhCn","zTree","zTreecheck","public","bootstrapdatetimepicker","bootstrapdatetimepickeri18n"], function ($) {
	...
	$(function() {
		...
		$("#checkProid").click(function(){
	    	$.ajax({
	    		url : "save?id="+$("#id").val()+"&orderno="+$("#orderno2").val()+"&accounts="+$("#accounts").val()+"&type="+$("#type").val()+"&status="+$("#status").val(),
	    		type : 'post',
	    		dataType : "json",
	    		async : true,
	    		error : function() {
		...
		$("#edit").on("click",function(e){
		    ...
		    if(data.length == 0){
		    	...
		    	$("#orderno2").attr("value",data[0].orderno);
		    	$("#accounts").attr("value",data[0].accounts);
