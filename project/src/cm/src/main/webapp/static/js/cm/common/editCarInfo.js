...
function initEditCarInfoScript() {
	...
	function dataBack(num){
		...
		win.find("#standardnamePageInfo"+num).text($("#standardname").val());//品牌型号
		win.find("#enginenoPageInfo"+num).text($("#engineno").val());//发动机号
		win.find("#vincodePageInfo"+num).text($("#vincode").val());//车辆识别代码
		win.find("#registdatePageInfo"+num).text($("#registdate").val().substring(0,11));//车辆初登日期
		win.find("#isTransfercarPageInfo"+num).text(($("#isTransfercar").val())=="1"?"是":"否");//是否过户车
		win.find("#mileagevaluePageInfo"+num).text($("#MILEAGEVALUE_"+$("#mileage").val()).text());//平均行驶里程
		win.find("#drivingareavaluePageInfo"+num).text($("#DRIVINGAREAVALUE_"+$("#drivingarea").val()).text());//行驶区域
		if($("#isTransfercar").val()=="1"){
			win.find("#transferdatePageInfo"+num).text($("#transferdateqtxt").val());//过户时间
		}
	}
	...
	$("#makesure").on("click",function(){
		...
		var carproperty = $('#carproperty option:selected').val();
		var syvehicletypename = $("#syvehicletypename").val();
		...
		if(carproperty=='6' || carproperty =='12'){
		...
		if(carproperty!='6' && carproperty !='12' && carproperty!='15' && carproperty!='16'){
...
//			$("#carproperty").append("<option value=11>非营业机关</option>");
	...
	$UpdateCarModelInfo = function(row){
		...
		//设置精友车型车辆种类
		$("#syvehicletypename").val(row.syvehicletypename);
	}
