...
<head>
	...
	<script type="text/javascript">
		requirejs(["cm/mytask/control","cm/manualprice/manualPrice","cm/manualprice/subMabualPrice"]);
	</script>
</head>
<body>
	...
	<div class="container-fluid manualtask">
			...
			<#list carInsTaskInfoList as carInsTaskInfo>
			    <div class="tab-pane fade <#if (carInsTaskInfo_index = 0)>in active</#if>" id="${carInsTaskInfo.inscomcode}" style="border-width:0px;">
			    	<form class="form-inline" role="form" id="insurancePolicyInfoForm${carInsTaskInfo_index}">
				    	<!--引入人工报价子页面   -->
				    	<#include "cm/common/subManualPrice.ftl"/>
			    	</form>
			    </div>
  			</#list>
		</div>
		...
		<input type="hidden" id="manualPriceAgentJobNum" value="${agentInfo.jobnum}"/>
		<input type="hidden" id="manualPriceDeptCode" value="${agentInfo.deptCode}"/>
		<input type="hidden" id="isFeeArea" value="${isfeeflag}"/>
        <!--精灵报价标识-->
