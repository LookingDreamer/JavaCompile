	...
	if(confirm("确认要删除吗？")){
		...
		if(typeof(imgid) == "undefined" || imgid=='' || imgid=='null'){
			...
			$.ajax({
				...
				data:{
					"imgid":imgid, 
					"taskid":$("#taskid_img").val(),
					"subInstanceId":$("#subInstanceId_img").val()
				},
				type:'POST', 
				async:false,
				dataType:'json',
...
function databack(data){
	...
	if($(window.top.document).find("#menu").css("display")=="none"){
        ...
        try{
            //$(window.frames[2].document).find(".imglist").empty();
        ...
        try{
            //$(window.frames[1].document).find(".imglist").empty();
	...
	closepic();
	setTimeout(function(){
						if(parent.window.fra_orderquery){
							parent.window.fra_orderquery.location.reload();
						}
					},1000);
}
function closepic(){
...
<div class="container-fluid">
	...
	<div class="modal-footer">
		<div class="caption" style="text-align:center;">
			...
			<input id="taskid_img" type="hidden" value="${taskid}">
			<input id="subInstanceId_img" type="hidden" value="${subInstanceId}">
			<input id="imglistsize" type="hidden" value="${imageList?size}">
			<a href="#" onclick="deleteimg();"><i id="zoomin" class="glyphicon glyphicon-trash m-left-5" ></i></a>
